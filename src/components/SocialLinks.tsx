...
import { withAlpha, luminanceOf } from '../utils/format';
...
export default function SocialLinks({ links, maxVisible = 5, variant = 'auto' }: Props) {
  ...
  const setAndroidNavButtons = async (themeCard: string) => {
    if (Platform.OS !== 'android') return;
    const btnStyle = luminanceOf(themeCard) > 0.55 ? 'dark' : 'light';
    await NavigationBar.setButtonStyleAsync(btnStyle as 'light' | 'dark').catch(() => {});
  };

  useEffect(() => {
    setAndroidNavButtons(theme.colors.card);
  }, [theme.colors.card]);
  ...
  return (
    <View accessibilityRole="toolbar" accessibilityLabel="Redes sociais da rádio">
      ...
      <Modal visible={showMore} transparent animationType="fade" onRequestClose={() => setShowMore(false)} presentationStyle="overFullScreen">
        <StatusBar style={theme.statusBarStyle || 'light'} translucent backgroundColor="transparent" />

        <Pressable style={s.backdrop} onPress={() => setShowMore(false)} />
        <View pointerEvents="none" style={[s.navScrim, { backgroundColor: theme.colors.card }]} />

        <View style={[s.sheet, { backgroundColor: theme.colors.card, borderColor: withAlpha(theme.colors.primary, 0.45) }]} accessibilityViewIsModal accessibilityLabel="Outras redes sociais">
          {overflow.map((it) => (
            <Pressable
              key={it.key}
              onPress={() => {
