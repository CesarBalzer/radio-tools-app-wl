...
import {Modal, View, Text, Image, TouchableOpacity, TouchableWithoutFeedback, Linking, StyleSheet, ScrollView, Platform} from 'react-native';
...
function upscaleItunesArt(url?: string | null) {
	...
	// artworkUrl100 -> 600x600 melhora nitidez no modal
...
export default function TrackModal({visible, onClose, track}: Props) {
	...
	const price = track.trackPrice != null && track.currency ? `${track.currency} ${track.trackPrice}` : null;
		...
		<Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
			...
			<View style={styles.sheet}>
				<ScrollView bounces={false} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
					...
					{!!track.primaryGenreName && <Text style={[styles.subtle, styles.italic]}>{track.primaryGenreName}</Text>}

					{/* Meta */}
					{!!released && <Text style={styles.meta}>Lançamento: {released}</Text>}

					{/* {!!price && <Text style={styles.meta}>Preço: {price}</Text>} */}

					{/* Actions */}
					{/* {!!track.trackViewUrl && (
						<TouchableOpacity style={styles.primaryBtn} onPress={openAppleMusic}>
							<Ionicons name="musical-notes" size={18} color="#0B0A10" />
							<Text style={styles.primaryBtnText}>Abrir no Apple Music</Text>
						</TouchableOpacity>
					)} */}
...
function makeStyles(theme: ReturnType<typeof useTheme>) {
	...
	return StyleSheet.create({
		...
		primaryBtn: {
			...
			borderRadius: 12,
			backgroundColor: colors.primary, // ex.: #7C3AED (Aurora)
			flexDirection: 'row',
			alignItems: 'center',
			gap: 10,
			alignSelf: 'stretch',
		...
		primaryBtnText: {
			fontSize: 16,
			fontWeight: '700',
			// texto escuro para bom contraste sobre o roxo claro do Aurora
			color: '#0B0A10'
		},
		secondaryBtn: {
			marginTop: 10,
